      ...
      <div class="main-content">
        <div class="visualization-panel">
          <Visualizer 
            ...
            bestFitness={statistics().currentBest?.fitness ?? 0}
            averageFitness={statistics().averageFitness[statistics().averageFitness.length - 1] ?? 0}
          />
        </div>
          ...
          <div class="controls">
            ...
            <div class="progress">
              <div class="progress-label">
                進行状況: {statistics().generation} / {DEFAULT_CONFIG.generations}
              </div>
              ...
              <svg width="300" height="200" viewBox="0 0 300 200">
                ...
                <g class="chart-data">
                  {statistics().bestFitness.length > 1 && (
                    <>
                      <polyline
                        points={statistics().bestFitness.map((fitness, i) => {
                          const x = 30 + (i / (DEFAULT_CONFIG.generations - 1)) * 250;
                          const maxFitness = Math.max(...statistics().averageFitness);
                      ...
                      />
                      <polyline
                        points={statistics().averageFitness.map((fitness, i) => {
                          const x = 30 + (i / (DEFAULT_CONFIG.generations - 1)) * 250;
                          const maxFitness = Math.max(...statistics().averageFitness);
                          const y = 170 - (fitness / maxFitness) * 150;
                          return `${x},${y}`;
                ...
                <g class="chart-labels">
                  <text x="155" y="195" text-anchor="middle" class="axis-label">世代</text>
                  <text x="15" y="100" text-anchor="middle" class="axis-label" transform="rotate(-90 15 100)">適応度</text>
                </g>
              </svg>
              <div class="legend">
