...
import { GeneticAlgorithm, Individual, GAConfig, DEFAULT_CONFIG } from './genetic-algorithm';
...
describe('GeneticAlgorithm', () => {
  ...
  describe('Sphere Function', () => {
    test('should calculate sphere function correctly', () => {
      ...
      const sphereFunction = (ga as any).sphereFunction.bind(ga);
  ...
  describe('Individual Creation', () => {
    test('should create individuals within bounds', () => {
      ...
      for (let i = 0; i < 100; i++) {
        const individual = (gaWithBounds as any).createRandomIndividual();
        ...
        expect(individual.fitness).toBe(individual.x * individual.x + individual.y * individual.y);
      }
    });

    test('should create individuals with correct fitness', () => {
      const individual = (ga as any).createRandomIndividual();
      const expectedFitness = individual.x * individual.x + individual.y * individual.y;
  ...
  describe('Gaussian Random', () => {
    test('should generate numbers with approximately normal distribution', () => {
      ...
      for (let i = 0; i < 1000; i++) {
        samples.push((ga as any).gaussianRandom());
      }
  ...
  describe('Tournament Selection', () => {
    test('should select better individual from tournament', () => {
      ...
      (ga as any).population = population;
      (ga as any).config.tournamentSize = 2;
      
      // Run tournament selection multiple times and check it tends to select better individuals
      const results: number[] = [];
      for (let i = 0; i < 100; i++) {
        const selected = (ga as any).tournamentSelection();
        results.push(selected.fitness);
      }
      
      // The average fitness of selected individuals should be better than random selection
      const averageSelected = results.reduce((sum, f) => sum + f, 0) / results.length;
      const averagePopulation = population.reduce((sum, ind) => sum + ind.fitness, 0) / population.length;
  ...
  describe('BLX-Alpha Crossover', () => {
    test('should produce children within expected range', () => {
      ...
      const [child1, child2] = (ga as any).blxAlphaCrossover(parent1, parent2, alpha);
    ...
    test('should respect bounds constraint', () => {
      ...
      const [child1, child2] = (gaWithBounds as any).blxAlphaCrossover(parent1, parent2);
  ...
  describe('Gaussian Mutation', () => {
    test('should only mutate when random < mutation rate', () => {
      ...
      const mutated = (gaNoMutation as any).gaussianMutation(individual);
    ...
    test('should respect bounds after mutation', () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        const mutated = (gaAlwaysMutate as any).gaussianMutation(individual);
  ...
  describe('Configuration', () => {
    test('should use custom configuration', () => {
      ...
      expect((customGA as any).config).toEqual(customConfig);
    });

    test('should use default configuration when none provided', () => {
      const defaultGA = new GeneticAlgorithm();
      expect((defaultGA as any).config).toEqual(DEFAULT_CONFIG);
    });
  });
});